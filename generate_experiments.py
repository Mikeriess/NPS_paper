...
for day in df.index:
    if df.loc[day, "burn_in"]  <= df.loc[day, "days"]:
        df.loc[day, "days"] = df.loc[day, "days"] + df.loc[day, "burn_in"]
...
df.burn_in = df.burn_in.astype(int)
