...
def store_evlog(L, P_scheme, agents, filedest, burn_in, NPS_bias, seed):
    ...
    from queue_prioritization.distributions.tNPS import simulate_NPS
    ...
    for case in L:
        ...
        if case["status"] == "closed":
            ...
            if length > 0:
                ...
                # Calculate average start delay and duration delay
                case["avg_start_delay"] = sum(case["t_start_delay"]) / len(case["t_start_delay"])
                case["avg_duration_delay"] = sum(case["t_delayed"]) / len(case["t_delayed"])
                ...
                res_i = pd.DataFrame({"case_id":[case["i"]]*length,
                                      ...
                                      "activity_start_delay":[case["avg_start_delay"]]*length,
