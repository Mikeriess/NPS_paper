# -*- coding: utf-8 -*-
"""
Web UI module for Queue Prioritization Simulation.

This module provides a Flask-based web interface for running queue prioritization 
...
from queue_prioritization.experiment.DoE import build_full_fact, fix_label_values
from queue_prioritization.algorithms.alg1_timeline_simulation import Run_simulation

# Create the application instance with correct template and static folders
app = Flask(__name__, 
           template_folder=os.path.join(os.path.dirname(__file__), "templates"),
           static_folder=os.path.join(os.path.dirname(__file__), "static"))

# Set the root path for experiments
...
def run_experiment():
    ...
    try:
        ...
        results_dir = os.path.join(EXPERIMENTS_DIR, "interactive_results")
...
def patched_run_simulation(agents, P_scheme, ceiling, ceiling_value, D, burn_in, seed, startdate, NPS_bias, output_dir=None):
    ...
    try:
        ...
        # Set a default filename if output_dir is provided
        if output_dir_abs:
            filename = os.path.join(output_dir_abs, f"{P_scheme}_seed{seed}_result.csv")
        else:
            filename = ""  # Empty filename will skip file saving
        
        # Run the simulation with the filename
        evlog, Case_DB = Run_simulation(
            ...
            startdate=startdate,
            NPS_bias=NPS_bias,
            filename=filename  # Pass the filename to Run_simulation
        )
        ...
        if output_dir_abs:
            ...
            # Save the event log and case database using the scheme as the file prefix
            evlog_file = os.path.join(output_dir_abs, f"{P_scheme}_log.csv")
            case_db_file = os.path.join(output_dir_abs, f"{P_scheme}_case_DB.csv")
            ...
            if os.path.exists(timeseries_path):
                ...
                timeseries_file = os.path.join(output_dir_abs, f"{P_scheme}_timeseries.csv")
...
def main():
    """Run the web UI."""
    print("Starting Queue Prioritization Simulation Web UI")
    print(f"Experiment results will be stored in: {EXPERIMENTS_DIR}")
